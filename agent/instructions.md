...
- `balance of exchange` → same (address book label)
- `check ETH on 0x…` → `get_balance({ address: "0x…" })`

**Wrong:** noticing the address/name in the message but calling `get_token_balances` / `get_balance` with empty args (that queries the UI wallet instead).
